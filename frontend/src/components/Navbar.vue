<template>
  <header class="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-slate-200 bg-white/90 px-4 backdrop-blur sm:px-6">
    <div class="flex items-center gap-3">
      <button class="rounded-xl border border-slate-200 p-2 text-slate-600 hover:bg-slate-50 lg:hidden" aria-label="Open menu" @click="$emit('menu')">☰</button>
      <div>
        <p class="text-xs font-semibold uppercase tracking-wider text-slate-400">Workspace</p>
        <h1 class="text-base font-black text-slate-900 sm:text-lg">{{ currentRoute }}</h1>
      </div>
    </div>

    <div class="flex items-center gap-2 sm:gap-4">
      <router-link to="/notifications" class="relative rounded-xl p-2 text-slate-500 hover:bg-slate-100" aria-label="Notifications">
      ...
      <button class="rounded-xl border border-slate-200 px-3 py-2 text-xs font-bold text-slate-600 hover:bg-slate-50" @click="handleLogout">Logout</button>
    </div>
  </header>
</template>
