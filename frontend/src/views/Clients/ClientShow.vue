<template>
  <section>
    ...
    <div v-else-if="client" class="space-y-6">
      <div class="flex flex-col gap-4 rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200 sm:flex-row sm:items-center sm:justify-between"><div><p class="text-sm font-semibold text-indigo-600">Client</p><h2 class="text-3xl font-black">{{ client.name }}</h2><p class="mt-1 text-sm text-slate-500">{{ client.company || 'Independent client' }}</p></div><div class="flex flex-col gap-2 sm:flex-row"><a v-if="getWhatsAppUrl(client.phone)" :href="getWhatsAppUrl(client.phone)" target="_blank" rel="noopener noreferrer" class="rounded-xl bg-green-600 px-4 py-3 text-center text-sm font-bold text-white hover:bg-green-700">WhatsApp</a><router-link :to="`/clients/${client.id}/edit`" class="rounded-xl bg-indigo-600 px-4 py-3 text-center text-sm font-bold text-white">Edit Client</router-link></div></div>
